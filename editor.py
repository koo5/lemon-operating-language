...
from element import Font, Element, TemplateManager, Template, \
    TextElement, Rectangle
from lemon_logger import LemonLogger
from pyglet.gl.gl import glClearColor, glColor3f
...
class EditorWindow(pyglet.window.Window):
    ...
    def __init__(
        ...
        tm = TemplateManager()
        templates = [  # Dumb starter elements
            'for',
            'if',
            'while',
            'for2',
        ...
        for _ in range(20):
            a = TextElement(self.root_element, text='',
                            font_name='monospace')
    ...
    def on_draw(self):
        ...
        self.clear()
        self.root_element.draw(y_height=0, y_offset=int(self._height
                               + self.cursor_row))
    ...
    def drawCaret(self):
        active_x_offset = self.cursor_col \
            * self.active_element.getLetterWidth()
        ...
        active_width = self.active_element.getWidth()  # @UnusedVariable
        if self.caret_visible:
            glColor3f(1, 1, 1)
        else:
            glColor3f(0.2, 0.2, 0.2)
        Rectangle(active_x, active_y + active_height, active_x + 1,
                  active_y)
    ...
    def on_text(self, text):
        self.cursor_col += self.active_element.addText(text,
                self.cursor_col)
    ...
    def updateActiveElement(self):
        self.active_element = \
            self.root_element.getChild(self.cursor_row)
